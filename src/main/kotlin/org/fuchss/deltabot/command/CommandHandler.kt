 ...
 */
class CommandHandler(private val configuration: BotConfiguration, private val commandRegistry: ICommandRegistry) : EventListener {
    ...
    private fun handleSlashCommand(event: SlashCommandInteraction) {
        ...
        if (event.guild != null && command.permissions == CommandPermissions.GUILD_ADMIN && event.user !in configuration.getAdminMembersOfGuildWithGlobalAdmins(event.guild!!)) {
            event.reply("You are not an admin!").setEphemeral(true).queue()
            return
        }
